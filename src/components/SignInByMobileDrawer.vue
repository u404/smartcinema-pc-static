<template>
  <div class="sign-in-drawer">
    <div class="step step--enter form" v-form v-if="step === 0" key="step--enter">
      ...
      <button class="form_button" :disabled="smsButtonDisabled" type="submit" @click="sendSmsAndGoNextStep">{{ smsCodeCountdown > 0 ? `${smsCodeCountdown}s` : 'Send Code' }}</button>
    ...
    <div class="step step--sign-in form" v-form v-else key="step--sign-in">
      ...
      <div class="button link-button back-button" @click="back"></div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async sendSmsAndGoNextStep () {
      ...
      try {
        ...
        this.step++
        logger.log("login_dialog_mobile", { tag: this.tag })
      } catch (e) {
...
@import "~@/common/style/sign-in-drawer.scss";
</style>
